...
6. Tích hợp OpenTelemetry Tracing Spans & Prometheus Telemetry Metrics.
==============================================================================
"""
...
def fetch_baseline_data_from_trino(feature_name: str, sample_size: int) -> Optional[np.ndarray]:
    """Truy vấn dữ liệu Baseline tương ứng từ Delta Lake Gold Layer thông qua Trino Engine (Trace Span)."""
    if os.getenv("TESTING") == "1":
        return None
    with trace_span("trino.fetch_baseline_drift", {"feature_name": feature_name, "sample_size": sample_size}):
        try:
...
async def fetch_stream_data_from_redis(feature_name: str, sample_size: int) -> Optional[np.ndarray]:
    """Rút dữ liệu Streaming thời gian thực tương ứng từ Redis Online Store (Trace Span)."""
    if os.getenv("TESTING") == "1":
        return None
    with trace_span("redis.fetch_stream_drift", {"feature_name": feature_name, "sample_size": sample_size}):
        try:
...
class HealthCheckResponse(BaseModel):
    ...
    service: str = Field(..., json_schema_extra={"example": "Drift Detection API"})
    timestamp: str = Field(...)

class FeatureDriftMetric(BaseModel):
    feature_name: str = Field(..., json_schema_extra={"example": "f_stream_views_30m"})
    p_value: float = Field(..., json_schema_extra={"example": 0.842})
    ks_statistic: float = Field(..., json_schema_extra={"example": 0.045})
    psi_score: float = Field(..., json_schema_extra={"example": 0.012})
    drift_status: str = Field(..., json_schema_extra={"example": "NO_DRIFT"})

class DriftAnalysisRequest(BaseModel):
    features: List[str] = Field(
        default=["f_stream_views_30m", "f_stream_add_to_cart_30m", "f_customer_avg_order_value_90d"],
        json_schema_extra={"example": ["f_stream_views_30m", "f_stream_add_to_cart_30m"]}
    )
    sample_size: int = Field(default=1000, json_schema_extra={"example": 1000})

class DriftAnalysisResponse(BaseModel):
    overall_status: str = Field(..., json_schema_extra={"example": "STABLE"})
    drifted_features_count: int = Field(..., json_schema_extra={"example": 0})
    metrics: List[FeatureDriftMetric] = Field(...)
    baseline_dataset: str = Field("Delta Lake Gold Layer (90d)", json_schema_extra={"example": "Gold Baseline"})
    current_stream: str = Field("Redis Real-Time Stream (30m)", json_schema_extra={"example": "Redis Stream"})
...
async def analyze_data_drift(request: DriftAnalysisRequest):
    ...
    """
    with trace_span("drift.analyze_features", {"features_count": len(request.features), "sample_size": request.sample_size}):
        metrics = []
        drift_count = 0
